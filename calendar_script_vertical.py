...
date_labels = [date.strftime('%Y-%m-%d') for date in min(hourly_average_apm.keys() + timedelta(hours=1)).strftime('%H:%M:%S') for _ in range(24)]

# Plot the heatmap
plt.figure(figsize=(12, 8))
sns.heatmap(heatmap_data, cmap='YlGnBu', annot=True, fmt=".1f", cbar_kws={'label': 'Average APM'},
            xticklabels=range(24), yticklabels=date_labels)
